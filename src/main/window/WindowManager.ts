import { BrowserWindow, globalShortcut, ipcMain, screen, session, desktopCapturer } from 'electron';
...
export class WindowManager {
  ...
  public setupIpcHandlers(): void {
    ...
    ipcMain.handle('toggle-window-visibility', async (): Promise<{ success: boolean; error?: string }> => {
      try {
        if (!this.mainWindow || this.mainWindow.isDestroyed()) {
    ...
    ipcMain.handle('update-sizes', async (): Promise<{ success: boolean; error?: string }> => {
      try {
        ...
        try {
          ...
          layoutMode = await this.mainWindow.webContents.executeJavaScript(
        ...
        await this.animateWindowResize(targetWidth, targetHeight, `${viewName} view (${layoutMode})`);
  ...
  private calculateWindowSize(viewName: ViewType, layoutMode: string): { targetWidth: number; targetHeight: number } {
  ...
  private async animateWindowResize(targetWidth: number, targetHeight: number, description: string): Promise<void> {
    if (!this.mainWindow || this.mainWindow.isDestroyed()) {
