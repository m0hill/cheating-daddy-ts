...
class Application {
  ...
  private setupGeneralIpcHandlers(): void {
    ...
    ipcMain.handle('open-external', async (_, url: string): Promise<{ success: boolean; error?: string }> => {
      try {
    ...
    ipcMain.handle('update-content-protection', async (): Promise<{ success: boolean; error?: string }> => {
      try {
        if (this.windowManager) {
          const contentProtection = await this.windowManager.getMainWindow()?.webContents.executeJavaScript(
