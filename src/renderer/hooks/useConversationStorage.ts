...
export function useConversationStorage() {
  ...
  const getConversationSession = useCallback(async (sessionId: string): Promise<ConversationSession | null> => {
    if (!conversationDB) {
      await initConversationStorage();
    }
  ...
  const deleteConversationSession = useCallback(async (sessionId: string): Promise<void> => {
    if (!conversationDB) {
      await initConversationStorage();
    }
