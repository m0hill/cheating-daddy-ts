...
export function useMediaCapture() {
  ...
  const addTokens = useCallback((count: number, type: 'image' | 'audio' = 'image') => {
    const now = Date.now();
  ...
  const shouldThrottle = useCallback((): boolean => {
    ...
    cleanOldTokens();
    const currentTokens = tokenTrackerRef.current.tokens.reduce((total, token) => total + token.count, 0);
  ...
  const setupLinuxMicProcessing = useCallback(async (micStream: MediaStream) => {
    ...
    micProcessor.onaudioprocess = async (e) => {
      ...
      while (audioBuffer.length >= samplesPerChunk) {
        ...
        await electronAPI.invoke.sendAudioContent({ data: base64Data, mimeType: 'audio/pcm;rate=24000' });
      }
    };
  ...
  const captureScreenshot = useCallback(async (
    ...
    if (hiddenVideoRef.current.readyState < 2 || !offscreenContextRef.current || !offscreenCanvasRef.current) {
      console.warn('Video or canvas not ready, skipping screenshot');
      return;
    }

    offscreenContextRef.current.drawImage(hiddenVideoRef.current, 0, 0, offscreenCanvasRef.current.width, offscreenCanvasRef.current.height);
    ...
    offscreenCanvasRef.current.toBlob(
      async (blob) => {
        ...
        reader.onloadend = async () => {
          ...
          if (!base64data || base64data.length < 100) return console.error('Invalid base64 data generated');

          const result = await electronAPI.invoke.sendImageContent({ data: base64data });
          if (result.success) {
            const imageTokens = calculateImageTokens(offscreenCanvasRef.current!.width, offscreenCanvasRef.current!.height);
  ...
  const stopCapture = useCallback(async () => {
    try {
        ...
        setState(prev => ({ ...prev, error: error instanceof Error ? error.message : 'Failed to stop capture' }));
    }
  ...
  const captureManualScreenshot = useCallback(async (imageQuality?: ImageQuality) => {
  ...
  const startCapture = useCallback(async (
    ...
    setState(prev => ({ ...prev, isCapturing: true, error: null, currentImageQuality: imageQuality }));
    ...
    try {
        ...
        if (isMacOS) {
            await electronAPI.invoke.startMacOSAudio();
             ...
             try {
                const micStream = await navigator.mediaDevices.getUserMedia({ audio: { sampleRate: SAMPLE_RATE, channelCount: 1, echoCancellation: true, noiseSuppression: true, autoGainControl: true }, video: false });
                await setupLinuxMicProcessing(micStream);
            } catch (micError) {
                console.warn('Failed to get microphone access on Linux:', micError);
            }
        } else { // Windows
            displayMediaOptions.audio = { sampleRate: SAMPLE_RATE, channelCount: 1, echoCancellation: true, noiseSuppression: true, autoGainControl: true };
        }
        ...
        if (screenshotIntervalSeconds !== 'manual') {
            ...
            screenshotIntervalRef.current = setInterval(() => captureScreenshot(imageQuality), intervalMs);
            setTimeout(() => captureScreenshot(imageQuality), 100);
        }
    } catch (error) {
        console.error('Error starting capture:', error);
        setState(prev => ({ ...prev, isCapturing: false, error: error instanceof Error ? error.message : String(error) }));
        await stopCapture();
    }
  }, [isMacOS, isLinux, electronAPI.invoke, setupLinuxMicProcessing, setupWindowsLoopbackProcessing, captureScreenshot, stopCapture]);
